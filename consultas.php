...
$query="insert into  tconsultas(nombres,email,cel,edad,fecha,hora,tconsulta,) values(?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('sssssss',$nombres,$email,$cel,$edad,$fecha,$hora,$tconsulta,);
$stmt->execute();
echo"<script>alert('Registro Satisfactorio para tu Consulta');</script>";
}
?>
...
$uid=$_SESSION['login'];
				...
				if($rs)
				{ ?>
			<h3 style="color: red" align="left">Tienes Reservas</h3>
				<?php }
				else{
							echo "";
							}			
...
<label class="col-sm-4 control-label"><h4 style="color: green" align="left">Información relacionada a tu Reserva </h4> </label>
</div>
